...
function saveConsultation(){
    ...
    data.push({name:'abstract1',value:$("#consultationAbstract").val()});
    data.push({name:'peea',value:$("#peea").val()});
    data.push({name:'ef',value:$("#ef").val()});
    
    //Activity List
    data.push({name:'activitySize',value:$("#tblSelectedActivities").DataTable().rows().data().length}); // the number of activities in the table
    $("#tblSelectedActivities").DataTable().rows().data().each(function(value,index){
        data.push({name:'activity'+index,value:value["idActivity"]});
        data.push({name:'activityPrice'+index,value:value["activityCost"]});    //price for current consultation
...
function initializeSibilingsTable(){
    ...
    $('#sibilingsTable').DataTable({
        ...
        "ordering":false,
        "paging":false,
        "ajax":"/demo/consultation/getPatientSibilings",
        "columns":[
...
function openSibilingFile(){
    var row = $("#sibilingsTable").DataTable().row('.selected').data();
    
    if(checkNotUndefined(row)){
        var box = bootbox.confirm("Desea abrir el expediente del paciente?", function(result) {
                    if(result){
                        window.open("/demo/patients/file/"+row["idPatient"]);
                    }
                  });
    }else{
...
var box = bootbox.confirm("Desea cancelar la consulta?", function(result) {
                    if(result){
                        $.ajax({
                            url:"/demo/consultation/cancelConsultation",
                            type:"POST",
                            success:function(response){
                                window.location.href ="/demo/";
                            }
