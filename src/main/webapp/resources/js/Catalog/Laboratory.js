...
function initializeMainTable(){
    $("#tblMain").DataTable({
        ...
        "ajax":"/demo/catalogs/getLaboratoryTest",
        "columns":[
            {"data":"laboratoryTest"},
...
function initializeAdditionalInfoTable(){
        ...
        "ajax":{
            "url":"/demo/catalogs/getLaboratoryTestRelatedInfo",
            "data":function(){
                return ({id:$("#tblMain").DataTable().row('.selected').data()["idLaboratoryTest"]});
            }
        },
        "columns":[
            {"data":"fatherLastName"},
...
function saveNewItem(){
    ...
    $.ajax({
        url:"/demo/catalogs/saveNewLaboratoryTest",
        data:data,
...
function modifyItem(){
    ...
    if(checkNotUndefined(row)){
        $("#inputModifyItem").val(row["laboratoryTest"]);
        if(row["active"] == 1 ){
            $("#inputModifyItemActive").prop('checked',true);
        }else{
            $("#inputModifyItemActive").prop('checked',false);
        }
        $("#inputIdItem").val(row["idLaboratoryTest"]);
        
        $('#mainTabMenu a[href="#tabModify"]').tab('show');
    }else{
        displayWarningAlert("No ha seleccionado un estudio");
    }
}
...
function saveModifyItem(){
    ...
    $.ajax({
        url:"/demo/catalogs/saveModifyLaboratoryTest",
        data:data,
        success:function(response,textStatus,jqXHR){
